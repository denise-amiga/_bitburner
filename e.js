...
export async function main(ns) {
  ...
  ns.disableLog("ALL");
  let servers = serverList(ns).filter((f) => !f.includes("zz-") && !f.includes("home"));
  for (let s of servers) {
    ...
    ns.print(ns.getServerRequiredHackingLevel(s).toString().padStart(4, " "), ":", ns.getServerNumPortsRequired(s), " >> ", s);
  }
}
